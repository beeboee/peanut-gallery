class PeanutGalleryCard extends HTMLElement {
  ...
  updateNavigationButtons() {
    ...
    if (card) card.classList.toggle("shuffle-mode", this.shuffleMode);

    if (previous) {
      previous.hidden = !hasImage || (this.shuffleMode && !canGoBackInShuffle);
      previous.title = this.shuffleMode ? "Previous shuffled comic" : "Previous comic";
    }

    if (next) {
      next.hidden = !hasImage;
      if (this.shuffleMode) {
        const canGoForwardInHistory = this.shuffleHistory.length > 0 && this.shuffleIndex >= 0 && this.shuffleIndex < this.shuffleHistory.length - 1;
        next.title = canGoForwardInHistory ? "Next shuffled comic" : "Shuffle";
        next.querySelector("ha-icon")?.setAttribute("icon", canGoForwardInHistory ? "mdi:arrow-right" : "mdi:shuffle-variant");
      } else {
        next.title = "Next comic";
  ...
  showRandom() {
    ...
    if (this.shuffleMode && this.sameDateShuffle) {
      data.same_date = true;
  ...
  updateSameDateButton() {
    ...
    if (!button) return;
    button.hidden = !this.shuffleMode;
    button.classList.toggle("active", this.sameDateShuffle);
    button.title = this.sameDateShuffle ? "Date-lock shuffle on" : "Date-lock shuffle off";
  }
  ...
  renderBase() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .next { position: absolute; top: 8px; right: 8px; z-index: 5; }
        .date-label { position: absolute; left: 8px; bottom: 8px; z-index: 4; padding: 6px 10px; border: 0; border-radius: 999px; background: rgba(0, 0, 0, 0.55); color: white; font: inherit; font-size: 13px; line-height: 1; cursor: pointer; }
        ...
        .menu-panel { display: grid; grid-template-columns: repeat(2, 42px); gap: 8px; justify-content: end; margin-bottom: 8px; }
        .time-machine { position: relative; }
        input[type="date"] { position: absolute; inset: 0; width: 100%; height: 100%; opacity: 0; cursor: pointer; }
        [hidden],
        ha-card.controls-hidden .previous, ha-card.controls-hidden .next, ha-card.controls-hidden .date-label, ha-card.controls-hidden .menu,
        ha-card.no-image .previous, ha-card.no-image .next, ha-card.no-image .date-label, ha-card.no-image .menu { display: none !important; }
      </style>
      ...
      <ha-card class="no-image">
        ...
        <button class="date-label" title="Today" type="button"></button>
        <details class="menu">
          ...
          <div class="menu-panel">
            ...
            <button class="menu-action shuffle-mode-toggle" type="button" title="Shuffle mode"><ha-icon icon="mdi:shuffle"></ha-icon></button>
            <button class="menu-action same-date-toggle" type="button" title="Date-lock shuffle" hidden><ha-icon icon="mdi:calendar-lock"></ha-icon></button>
    ...
    this.$(".date-label").addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); this.showToday(); });
    this.$(".shuffle-mode-toggle").addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); this.toggleShuffleMode(); });
